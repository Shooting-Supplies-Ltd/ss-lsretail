...
export async function getItems() {
  ...
  return axios.get(`Item.json?load_relations=["Category", "Images", "ItemShops", "CustomFieldValues"]&ItemECommerce.listOnStore=true`, axiosConfig).catch(err => console.error(err.data))
}

export async function getItem(itemID) {
  let axiosConfig = await getHeader()
  return axios.get(`Item/${itemID}.json?load_relations=["Category", "Images", "ItemShops", "CustomFieldValues", "ItemECommerce"]&ItemECommerce.listOnStore=true`, axiosConfig).catch(err => console.error(err.data))
}
...
export async function getClothing() {
  let axiosConfig = await getHeader()
  return axios.get(`Item.json?load_relations=["ItemShops", "Category", "Images", "ItemECommerce"]&ItemShops.qoh=%3E%3D,1&Category.categoryID=IN,[5]&ItemECommerce.listOnStore=true`, axiosConfig).catch(err => console.error(err.data))
}
...
export async function getManufacturers(manufacturerID) {
  let axiosConfig = await getHeader()
  return axios.get(`Manufacturer.json?manufacturerID=IN,${manufacturerID}`, axiosConfig).catch(err => console.error(err.data))
}
